...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
  ...
  vec4 maskColor = texture(iChannel1, uv);

  if (isMix == true)
  {
    vec4 bgColor = texture(iChannel2, uv);
    fragColor = mix(bgColor, orgColor, maskColor.r);
  }
  else
  {
...
function KuruNodeKit.createMaskNode(scene, maskFile, bgSampler, stretch)
  if stretch == nil then
    stretch = KuruBackgroundImageNodeStretch.FILL_HORIZONTAL
  end
  maskBn = KuruFrameBufferNode.create()
  scene:addNodeAndRelease(maskBn)
  maskBn:addChildAndRelease(KuruNodeKit.createBGNode(maskFile, {stretchType = stretch}))
  ...
  node:setChannel1(maskBn:getSampler())

  if bgSampler ~= nil then
    node:getMaterial():getParameter("isMix"):setBool(true)
    node:setChannel2(bgSampler)
  end
  -- scene:addNodeAndRelease(KuruResourceReleaseNode.create(maskBn))
