...
function UIView:new(rect, texturePath, enableOnlyPreview, enabled, enableDeliverNextResponder)
  local newView = {}
...
function UIView:updateAlpha(alpha)
  if (self.node ~= nil) then
    self.node:setModulateAlpha(alpha)
  end
end
...
function UIView:updateLayout(parentX, parentY)
  if (self.node ~= nil) then
    ...
    print("updateRect : "..tostring(vertexRect.x) .. ", "..tostring(vertexRect.y))
...
function UIView:isTouched(touchPoint)
  ...
  local leftX = self.rect.x / self.baseWidth
  local bottomY = self.rect.y / self.baseHeight
  local width = self.rect.width / self.baseWidth
  local height = self.rect.height / self.baseHeight
...
--         scale = 1.0
--       else
--         local deviceRatio = DeviceConfig.instance():getResolution().y / DeviceConfig.instance():getResolution().x
--         scale = 1.777778 / deviceRatio
--       end
--     end
--
